...
makeplotfunction <- function(i,cname){ 
  ...
  print("plot!")
  ggsave(paste("bi-weekly/", tit, "/top3/Top3__", cname, ".png", sep = ""), A1)
  return(A1) #!
}
...
gridfunction <- function(i,j){
  title1=textGrob(paste("Top 3 products (", tit,")",sep=""), gp=gpar(fontsize=20, font=2), just = "top")#fontface="bold")
  B1 <- grid.arrange(ncol = 2,grobs=c(RE[i:(i+3)]),top = title1)
  ggsave(paste("bi-weekly/", tit, "/top3/00",j,".png",sep=""),B1,scale=2)
  return(B1) #!
}
...
#j <- "03"
#ggsave(paste("./report/bi-weekly/20170206-20170217/top3/00",j,".png",sep=""),B1,scale=2)
